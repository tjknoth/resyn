...
reconstructI' env t@ScalarT{} impl = case impl of
  ...
  PIf (Program PHole AnyT) iThen iElse -> do
    ...
    -- env1, env2 <- share
    -- env2: 
    pThen <- inContext (\p -> Program (PIf (Program PHole boolAll) p (Program PHole t)) t) $ cleanExplore 
      $ reconstructI (addAssumption cUnknown env) t iThen
    cond <- conjunction <$> currentValuation cUnknown
    -- env1:
    pCond <- inContext (\p -> Program (PIf p uHole uHole) t) $ generateConditionFromFml env cond
    -- clear constraints generated by then branch before generating else
    -- env2: 
    pElse <- optionalInPartial t $ inContext (\p -> Program (PIf pCond pThen p) t) $ cleanExplore 
      $ reconstructI (addAssumption (fnot cond) env) t iElse
    return $ Program (PIf pCond pThen pElse) t
  
  PIf iCond iThen iElse -> do
    -- env1, env2 <- split
    (env1, env2) <- shareContext env $ show $ text "if" <+> pretty iCond
    -- env1:
    pCond <- inContext (\p -> Program (PIf p (Program PHole t) (Program PHole t)) t) $ reconstructETopLevel env1 (ScalarT BoolT ftrue defPotential) iCond
    let (env2', ScalarT BoolT cond pot) = embedContext env2 $ typeOf pCond
    -- env2: (clean)
    pThen <- inContext (\p -> Program (PIf pCond p (Program PHole t)) t) $ cleanExplore 
      $ reconstructI (addAssumption (substitute (Map.singleton valueVarName ftrue) cond) env2') t iThen
    -- env2: (clean)
    pElse <- inContext (\p -> Program (PIf pCond pThen p) t) $ cleanExplore 
      $ reconstructI (addAssumption (substitute (Map.singleton valueVarName ffalse) cond) env2') t iElse
    return $ Program (PIf pCond pThen pElse) t
  ...
  PMatch iScr iCases -> do
    ...
    -- env1, env2 <- split
    (env1, env2) <- shareContext env $ show $ text "match" <+> pretty iScr
    -- env1:
    pScrutinee <- inContext (\p -> Program (PMatch p []) t) $ reconstructETopLevel env1 scrT iScr
    let (env2', tScr) = embedContext env2 (typeOf pScrutinee)
    ...
    (x, env2'') <- addScrutineeToEnv env2' pScrutinee tScr
    -- env2: (clean)
    let exploreCase c consT = cleanExplore $ reconstructCase env2'' x pScrutinee t c consT
...
reconstructIE :: (MonadSMT s, MonadHorn s) => Environment -> RType -> UProgram -> Explorer s RProgram
reconstructIE env t impl = do 
  -- make sure this is clean:
  p <- cleanExplore $ reconstructETopLevel env t impl
...
reconstructETopLevel :: (MonadSMT s, MonadHorn s) => Environment -> RType -> UProgram -> Explorer s RProgram
...
reconstructE :: (MonadSMT s, MonadHorn s) => Environment -> RType -> UProgram -> Explorer s RProgram
...
reconstructE' env typ (PApp iFun iArg) = do
  ...
  -- env1:
  pFun <- inContext (\p -> Program (PApp p uHole) typ) $ reconstructE env1 (FunctionT x AnyT typ defCost) iFun
  ...
  pApp <- if isFunctionType tArg'
    then do -- Higher-order argument: its value is not required for the function type, enqueue an auxiliary goal
      ...
      -- env2:
      pArg <- inContext (\p -> Program (PApp pFun p) typ) $ reconstructE env2 tArg' iArg
      -- TODO: is env OK here?
...
checkAnnotation env t t' p = do
  ...
  case resolveRefinedType (typeSubstituteEnv tass env) t' potentialSyms of
    ...
    Right t'' -> do
      ...
      runInSolver $ solveTypeConstraints t'
