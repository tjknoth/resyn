...
-- Run an Explorer computation, clearing all typing constraints generated by the computation after
cleanExplore :: (MonadHorn s, MonadSMT s) => Explorer s a -> Explorer s a
cleanExplore go = do 
  rcs <- use (typingState . resourceConstraints)
  p <- go 
  (typingState . resourceConstraints) .= rcs
  return p
