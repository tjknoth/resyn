...
synthesize :: ExplorerParams -> HornSolverParams -> Goal -> [Formula] -> [Formula] -> IO (Either ErrorMessage [(RProgram, Int)], TimeStats)
synthesize explorerParams solverParams goal cquals tquals = evalZ3State $ evalFixPointSolver reconstruction solverParams
  where
    ...
    reconstruction :: HornSolver (Either ErrorMessage [(RProgram, Int)], TimeStats)
    reconstruction = let
